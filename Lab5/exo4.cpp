...
private:
    ...
    void rehash(bool __first_time__ = false)
    {
        int *old_data = data;
        bool *old_full = full;
        int old_data_size = __first_time__ ? 0 : 2 * table_size;
        ...
        while (!success)
        {
            ...
            data = new int[(table_size << 1)];
            full = new bool[(table_size << 1)];
            for (int i = 0; i < (table_size << 1); i++)
                full[i] = false;

            a1 = std::abs(std::rand()) % (table_size-1) + 1;
            a2 = std::abs(std::rand()) % (table_size-1) + 1;
            b1 = std::abs(std::rand()) % (table_size);
            b2 = std::abs(std::rand()) % (table_size);
            ...
            if (!__first_time__)
                for (int i = 0; i < old_data_size; i++)
                    if (old_full[i])
                    {
                        if (__insert__(old_data[i]).first == 2)
                        {
                            delete[] data;
                            delete[] full;
                            success = false;
    ...
    std::pair<int, int> compute_hashes(int key)
    {
        return {(((long long)(key * a1) + b1) % table_size + table_size) % table_size, table_size + (((long long)(key * a2) + b2) % table_size + table_size) % table_size};
    }

    std::pair<int, int> __insert__(int key)
    {
        if (count(key))
            return {0, key};
        ...
        for (int i = 0; i < 100; i++)
        {
            auto [hash_1, hash_2] = compute_hashes(key);
            ...
            if (i % 2)
            {
                int next = data[hash_2];
                data[hash_2] = key;
                key = next;
            }
            else
            {
                int next = data[hash_1];
                data[hash_1] = key;
                key = next;
            }
        }
...
public:
    ...
    int count(int key)
    {
        auto [index1, index2] = compute_hashes(key);

        return (full[index1] && data[index1] == key) || (full[index2] && data[index2] == key) ? 1 : 0;
    }
    ...
    bool erase(int key)
    {
        ...
        if (full[index1] && data[index1] == key)
        {
            full[index1] = false;
            set_size--;
            return true;
        }
        if (full[index2] && data[index2] == key)
        {
            full[index2] = false;
            set_size--;
            return true;
...
signed main()
{
    ...
    cout << "🧪 Testing the custom 'set' (Cuckoo Hashing) implementation.\n"
         << endl;
